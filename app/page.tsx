...
export default function LandingPage() {
  ...
  const { user } = useAuth();
  const dashboardHref =
    user?.accountType === "doctor" ? "/doctor-dashboard" : "/dashboard";
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <Link
                href={dashboardHref}
                className="inline-flex items-center h-9 px-4 rounded-xl bg-accent text-white text-sm font-medium hover:bg-accent-hover transition-colors"
              >
                Dashboard →
              </Link>
            ) : (
              <>
                <Link
                  href="/auth/login"
                  className="h-9 px-4 rounded-xl text-sm font-medium text-ink2 hover:bg-bg2 transition-colors inline-flex items-center"
                >
                  {t.landing.login}
                </Link>
                <Link
                  href="/auth/register"
                  className="h-9 px-4 rounded-xl bg-accent text-white text-sm font-medium hover:bg-accent-hover transition-colors inline-flex items-center"
                >
      ...
      <main className="flex-1">
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 pt-20 pb-24 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/auth/register"
              className="h-12 px-8 rounded-xl bg-accent text-white font-semibold text-base hover:bg-accent-hover transition-colors shadow-sm inline-flex items-center"
            >
              {t.landing.get_started}
            </Link>
            <a
              href="#how"
              className="h-12 px-8 rounded-xl border border-[#e7e5e4] text-ink2 font-medium text-base hover:bg-bg2 transition-colors inline-flex items-center gap-2"
            >
        ...
        <section id="features" className="bg-white border-y border-[#e7e5e4]">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-20">
            <div className="text-center mb-14">
              ...
              <p className="text-ink2 max-w-lg mx-auto">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {features.map((f) => (
                <div
                  key={f.key}
                  className="rounded-2xl border border-[#e7e5e4] p-6 hover:shadow-md transition-shadow bg-white"
                >
                  <div
        ...
        <section className="bg-ink">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 py-20 text-center">
            ...
            <Link
              href="/auth/register"
              className="inline-flex items-center h-12 px-8 rounded-xl bg-accent text-white font-semibold text-base hover:bg-accent-hover transition-colors"
            >
              {t.landing.cta_btn}
            </Link>
          </div>
      ...
      <footer className="border-t border-[#e7e5e4] bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <p className="text-xs text-ink3">
            © 2026 ANAM-AI. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
